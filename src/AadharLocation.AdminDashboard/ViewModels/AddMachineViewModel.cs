...
public partial class AddMachineViewModel : ObservableObject
{
    ...
    private async Task LoadOperatorsAsync()
    {
        try
        {
            var result = await _api.GetOperatorsAsync(pageSize: 100);
            AvailableOperators = result?.Items.ToList() ?? [];
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            {
                await _api.UpdateMachineAsync(_editingId.Value,
                    new UpdateMachineRequest(Name, Type, AssignedOperatorId, Status));
            }
            else
            {
                await _api.CreateMachineAsync(
                    new CreateMachineRequest(Name, SerialNumber, Type, AssignedOperatorId));
            }
